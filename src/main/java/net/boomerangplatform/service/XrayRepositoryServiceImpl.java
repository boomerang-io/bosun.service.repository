...
//		sb.append(this.settingsService.getConfiguration("cicd", "xray.url.api.base").getValue())
//				.append(xrayDependencyGraph);
		sb.append(xrayUrlApiBase).append(xrayDependencyGraph);

		String url = sb.toString();

		final ResponseEntity<DependencyGraph> response = internalRestTemplate.exchange(url, HttpMethod.POST, request,
	...
	public ArtifactSummary getArtifactSummary(String artifactPath, String artifactName, String artifactVersion) {
		...
		StringBuilder sb = new StringBuilder();
//		sb.append(this.settingsService.getConfiguration("cicd", "xray.url.api.base").getValue())
//				.append(xrayArtifactSummary);
		sb.append(xrayUrlApiBase).append(xrayArtifactSummary);

		String url = sb.toString();

		final ResponseEntity<ArtifactSummary> response = internalRestTemplate.exchange(url, HttpMethod.POST, request,
		...
		for (Component artifactComponent : rawDependencyGraph.getComponents()) {
			...
			if (artifactComponent.getComponents() != null) {
				for (Component packageComponent : artifactComponent.getComponents()) {
					...
					componentMap.put(packageComponent.getComponentName(), getComponent(componentMap, packageComponent));
				}
			}
		}
		return componentMap;
	...
	private String getPath(String artifactVersion, String artifactName, String artifactPath) {
		...
		StringBuilder sb = new StringBuilder();
		sb.append(boomerangArtifactoryId).append("/").append(boomerangRepoDocker).append("/").append(org).append("/")
				.append(artifactName).append("/").append(artifactVersion);
		return sb.toString();
	}

	private HttpHeaders getHeaders() {
//		final String plainCreds = this.settingsService.getConfiguration("cicd", "xray.boomerang.user").getValue() + ":"
//				+ this.settingsService.getConfiguration("cicd", "xray.boomerang.apitoken").getValue();
